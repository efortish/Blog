        ...
        >
            <Link to={`/blog/${data.slug}`}
                onMouseEnter={()=>{
                    ...
                    const tittle = document.getElementById(`tittle`+data.id)
                    tittle.classList.add('text-orange-500')
                }} 
                onMouseLeave={()=>{
                    // const img = document.getElementById(index)
                    // img.classList.remove('object-fill')
                    const tittle = document.getElementById(`tittle`+data.id)
                    tittle.classList.remove('text-orange-500')
                }}
              ...
              <div className="flex items-center   my-10 ">
                <div className="lg:flex min-w-0 lg:flex-1 items-center">
                  <figure className="lg:flex-shrink-0">
                    <img id={index} className="h-64 lg:w-96 w-full object-contain rounded" src={data.thumbnail} alt="" />
                  </figure>
                  <div className="min-w-0 flex-1 px-8 p-4 ">
                    <p id={`tittle`+data.id} className=" lg:mt-0 lg:absolute lg:top-4 leading-10 text-3xl pb-4 font-semibold transition duration-300 ease-in-out">{data.tittle.length > 80 ? data.tittle.slice(0,79):data.tittle}</p>
                    <div className="lg:absolute lg:top-28">
                        ...
                        <p className="mt-4 text-lg font-regular text-gray-800 leading-8">{data.excerpt.length > 150 ? data.excerpt.slice(0,149):data.excerpt}</p>
                    </div>
                  </div>
                </div>
              </div>
